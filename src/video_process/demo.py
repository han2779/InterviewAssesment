...
class FacialExpressionAnalyzer:
    ...
    def get_summary(self):
        ...
        self.results['smile_frequency'] = round(self.smile_count / minutes, 1)
        self.results['eye_contact_ratio'] = round(self.eye_contact.eye_contact_ratio, 2)
        self.results['eyebrow_intensity'] = round(self.eyebrow_analyzer.average_intensity, 1)
        self.results['blink_frequency'] = round(self.blink_detector.blink_count / minutes, 1)

        # 添加时间信息
        self.results['analysis_duration'] = round(elapsed_time, 1)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # 显示结果
            cv2.imshow('Facial Expression Analysis', processed_frame)
            ...
            if key == ord('s'):
                if analyzer.is_analyzing:
                    ...
                    summary = analyzer.get_summary()    # summary是个字典
                    if summary:
                        print("\n--- 面部表情分析摘要 ---")
                        print(f"分析时长: {summary['analysis_duration']}秒")
                        print(f"处理帧数: {summary['frame_count']}")
                        print(f"情绪分布: {summary['emotion_distribution']}")
                        print(f"微笑频率: {summary['smile_frequency']} 次/分钟")
                        print(f"眼神接触比例: {summary['eye_contact_ratio']}")
                        print(f"眉毛活动强度: {summary['eyebrow_intensity']}")
                        print(f"眨眼频率: {summary['blink_frequency']} 次/分钟")
                else:
